...
void signalHandler(int signum) {
  std::cout << "[INFO] interrupt signal (" << signum << ") received"
            << std::endl;
...
int main(int /*argc*/, char* /*argv*/[]) {
  ...
  if (envServerUrl == nullptr) {
    std::cerr << "[ERROR] missing 'SERVER_URL' environment variable"
              << std::endl;
  ...
  std::cout << "[INFO] verifying that room '" << envRoomId << "' exists..."
            << std::endl;
