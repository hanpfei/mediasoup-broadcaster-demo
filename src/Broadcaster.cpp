...
std::future<void> Broadcaster::OnConnect(mediasoupclient::Transport* transport,
  ...
  // std::cout << "[INFO] dtlsParameters: " << dtlsParameters.dump(4) <<
  // std::endl;
...
std::future<void> Broadcaster::OnConnectSendTransport(
    const json& dtlsParameters) {
  std::promise<void> promise;
  ...
  auto r = cpr::PostAsync(cpr::Url{this->baseUrl + "/broadcasters/" + this->id +
                                   "/transports/" +
...
std::future<void> Broadcaster::OnConnectRecvTransport(
    const json& dtlsParameters) {
  std::promise<void> promise;
...
void Broadcaster::OnConnectionStateChange(
  ...
  std::cout << "[INFO] Broadcaster::OnConnectionStateChange() [connectionState:"
...
std::future<std::string> Broadcaster::OnProduce(
  ...
  // std::cout << "[INFO] rtpParameters: " << rtpParameters.dump(4) <<
  // std::endl;
...
std::future<std::string> Broadcaster::OnProduceData(
  ...
  auto r =
      cpr::PostAsync(cpr::Url{this->baseUrl + "/broadcasters/" + this->id +
                              "/transports/" + this->sendTransport->GetId() +
                              "/produce/data"},
                     cpr::Body{body.dump()},
                     cpr::Header{{"Content-Type", "application/json"}},
                     cpr::VerifySsl{verifySsl})
...
void Broadcaster::CreateDataConsumer() {
  ...
  this->dataConsumer =
      this->recvTransport->ConsumeData(this, dataConsumerId, dataProducerId,
                                       streamId, "chat", "", nlohmann::json());
}
...
void Broadcaster::CreateSendTransport(bool enableAudio, bool useSimulcast) {
  ...
  this->sendTransport = this->device.CreateSendTransport(
      this, sendTransportId, response["iceParameters"],
...
void Broadcaster::CreateRecvTransport() {
  ...
  this->recvTransport = this->device.CreateRecvTransport(
      this, recvTransportId, response["iceParameters"],
      response["iceCandidates"], response["dtlsParameters"], sctpParameters);

  this->CreateDataConsumer();
}

void Broadcaster::OnMessage(mediasoupclient::DataConsumer* dataConsumer,
...
void Broadcaster::Stop() {
  ...
  cpr::DeleteAsync(cpr::Url{this->baseUrl + "/broadcasters/" + this->id},
                   cpr::VerifySsl{verifySsl})
      .get();
}
